...
Friend Module Misc
    ...
    Public Function FormatHTMLMessage(ByVal title As String, ByVal msg As String, ByVal redirectpage As String, ByVal timeout As String, ByVal critical As Boolean) As String
        If timeout = "" Then Throw New ArgumentException("TIMEOUT IS NULL!")
        If (timeout) = "0" Then Throw New ArgumentException("TIMEOUT IS NULL!")

        If title.Length > 2 Then
            title = title(0).ToString().ToUpper() & title.Remove(0, 1).ToLower()
        Else
            title = title.ToUpper()
        End If

        Dim q As String = "hide"
        Dim lnk As String = "#"
        If redirectpage.Length > 1 Then
            q = ""
            lnk = redirectpage
        End If

        Return My.Resources.messageTemplate.Replace("%ROOT%", WebRoot).Replace("%TITLE%", title) _
        .Replace("%SIGN%", BoardTitle & " - " & Now.ToString()) _
        .Replace("%BUTTON%", q) _
        .Replace("%LINK%", lnk) _
        .Replace("%MESSAGE%", msg)
            ...
            Else
                ...
                Try
                    ...
                    For i As Integer = 0 To signLength - 1 Step 1
                        'Dim byS As Byte = Convert.ToByte(signature(i), 16)
                        'If Not (byS = buff(i)) Then
                        '    boolResult = False
                        '    Exit For
                        'End If

                        Dim a As String = signature(i).ToUpper()
                        Dim b As String = buff(i).ToString("X2").ToUpper()
                    ...
                    If errorCount > (signature.Length / 2) Then
                        boolResult = False
                    Else
                        boolResult = True
                    End If
    ...
    Friend Sub Delete_File_Fromdisk(ByVal path As String)
