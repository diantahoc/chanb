...
function goleft(id) {
    var rotator = document.getElementById("rot" + id);
    if (rotator != null)
    {
        var active_index = 0;
        for (index = 0; index < rotator.children.length; index++)
        {
            if (rotator.children[index].getAttribute("class").indexOf("hide") == -1)
            {
                active_index = index;
                rotator.children[index].setAttribute("class", "hide"); //hide the file
                break;
            }
        }

        if (active_index == 0) // if it was the first file
        {
            active_index = rotator.children.length - 1; //switch to the last one
        }
        else
        {
            active_index--; //decrement by one
        }

        rotator.children[active_index].setAttribute("class", "file"); //show the file
    }
}
...
function goright(id) {
    ...
    if (rotator != null)
    {
        ...
        if (active_index == rotator.children.length - 1) // if it was the last file
        {
            active_index = 0; //switch to the first one
        }
        else
        {
            active_index++; //increment by one
        }
...
function gofirst(id) {
    ...
    if (rotator != null)
    {
        ...
        rotator.children[0].setAttribute("class", "file"); //show the first file
    }
}
...
function golast(id) {
    ...
    if (rotator != null)
    {
        ...
        rotator.children[rotator.children.length - 1].setAttribute("class", "file"); //show the last file
    }
}
