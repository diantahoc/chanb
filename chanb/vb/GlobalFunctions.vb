    ...
    Private Sub SavePostFile(ByVal f As HttpPostedFile, ByVal isReply As Boolean, ByVal postId As Integer, Optional ByVal Connection As DbConnection = Nothing)
        ...
        Select Case fileextension.ToUpper()
            Case "JPG", "JPEG", "PNG", "BMP", "GIF", "APNG"
                ...
                'Full image path
                Dim FullImagePath As String = CombinePath(StorageFolder, ChanbName & "." & fileextension)
                ...
                Dim isTransparant As Boolean = False

                'If fileextension = "png " Or fileextension = "apng" Then
                '    isTransparant = ImageHaveTransparentBits(CType(w, Drawing.Bitmap))
                'End If

                isTransparant = (fileextension = "png " Or fileextension = "apng")

                Dim format As System.Drawing.Imaging.ImageFormat

                If isTransparant Then
                    format = System.Drawing.Imaging.ImageFormat.Png
                    thumb = CombinePath(StorageFolderThumbs, "th" & ChanbName & ".png")
                Else
                    format = System.Drawing.Imaging.ImageFormat.Jpeg
                    thumb = CombinePath(StorageFolderThumbs, "th" & ChanbName & ".jpg")
                End If
                ...
                If (Not AllowDuplicatesFiles) And FileExistInDB(md5string, Connection) Then
                    ...
                    If Not SmartLinkDuplicateImages Then
            ...
            Case "SVG"
                ...
                Dim p As String = CombinePath(StorageFolder, dd & "." & fileextension)
                ...
                Else
                    ...
                    'Thumb path
                    Dim thumb As String = CombinePath(StorageFolderThumbs, "th" & dd & ".png")
                    ...
                    If isTransparant Then
                        format = System.Drawing.Imaging.ImageFormat.Png
                        thumb = CombinePath(StorageFolderThumbs, "th" & dd & ".png")
                    Else
                        format = System.Drawing.Imaging.ImageFormat.Jpeg
                        thumb = CombinePath(StorageFolderThumbs, "th" & dd & ".jpg")
                    End If
...
#If EnablePDF Then
                ...
                Dim p As String = CombinePath(StorageFolder, dd & "." & fileextension)
                'Thumb path
                Dim thumb As String = CombinePath(StorageFolderThumbs, "th" & dd & ".jpg")
                f.SaveAs(p)
...
#End If
            ...
            Case Else
                ...
                For i As Integer = 0 To CFH_Plugins.Count - 1 Step 1
                    ...
                    End If
                Next
                If Not fileHandled Then Throw New Exception("Unsupported file type '" & HttpUtility.HtmlEncode(fileextension) & "'")
        End Select
    End Sub
    ...
    Private Function ImageHaveTransparentBits(ByVal image As Drawing.Bitmap) As Boolean
        If EnchancedThumbGeneration Then
    ...
    Private Function GetCFHPlugins() As CustomFileHandler()
        ...
        For Each x As IO.FileInfo In FileIO.FileSystem.GetDirectoryInfo(chanb.My.Request.PhysicalApplicationPath & "\bin").GetFiles("plugin_cfh*.dll", IO.SearchOption.TopDirectoryOnly)
    ...
    Friend Function GetPostHTML(ByVal po As WPost, ByVal pa As HTMLParameters) As String
        ...
        Select Case po.type
            Case WPost.PostType.Thread
                ...
                If ShowThreadRepliesCount Then
                    ...
                    ' If replyC.TotalReplies > 0 Then finalHtml.Replace("%REPLY COUNT SPAN%", replyCountSpan.Replace("%REPLY COUNT%", CStr(replyC.TotalReplies))) Else finalHtml.Replace("%REPLY COUNT SPAN%", String.Empty)
                Else
                    postHTML = postHTML.Replace("%REPLY COUNT SPAN%", String.Empty)
                    '  finalHtml.Replace("%REPLY COUNT SPAN%", String.Empty)
                End If

            Case WPost.PostType.Reply
                postHTML = ReplyPostTemplate.Replace("%TID%", CStr(po.parent))

                If po.FileCount > 0 Then
                    postHTML = postHTML.Replace("%DFILESMENU%", deleteFilesHTMLMenuItem)
                Else
                    postHTML = postHTML.Replace("%DFILESMENU%", "")
                End If

                ' finalHtml.Append(ReplyPostTemplate)
            Case Else
                Return ""
        End Select
        ...
        If EnableUserID Then
            Dim idHt As String = UserIDHtmlSPAN.Replace("%UID%", po.posterID)


            postHTML = postHTML.Replace("%UID%", idHt)
            postHTML = postHTML.Replace("%UIDS%", po.posterID)

            'finalHtml.Replace("%UID%", idHt)
            'finalHtml.Replace("%UIDS%", po.posterID)

        Else
            postHTML = postHTML.Replace("%UID%", "")
            postHTML = postHTML.Replace("%UIDS%", "")
            'finalHtml.Replace("%UID%", "")
            'finalHtml.Replace("%UIDS%", "")
        End If

        If po.email = "" Then
            postHTML = postHTML.Replace("%NAMESPAN%", "<span class='name'>%NAME%</span>")
            ' finalHtml.Replace("%NAMESPAN%", "<span class='name'>%NAME%</span>")
        Else
            ...
            'finalHtml.Replace("%NAMESPAN%", "<a href='mailto:%EMAIL%' class='useremail'><span class='name'>%NAME%</span></a>")
            ' finalHtml.Replace("%EMAIL%", po.email)
        End If
        ...
        'finalHtml.Replace("%ID%", CStr(po.PostID))

        'finalHtml.Replace("%SUBJECT%", po.subject)
        'finalHtml.Replace("%NAME%", po.name)
        ''finalHtml.Replace("%DATE UTC UNIX%", CStr(po.time.ToFileTime))
        'finalHtml.Replace("%DATE UTC TEXT%", GetTimeString(po.time))

        'finalHtml.Replace("%LANG reportStr%", reportStr)
        'finalHtml.Replace("%LANG deleteStr%", deleteStr)
        'finalHtml.Replace("%ROOT%", WebRoot)
        ...
        If StaticHTML Then
            ...
            ' finalHtml.Replace("%POST LINK%", ThreadHTMLWebPath & parentPoID & ".html#p" & po.PostID)
        Else
            If po.archived Then
                If ConvertArchivedThreadToHTML Then
                    postHTML = postHTML.Replace("%POST LINK%", ArchivedTHTMLWebPath & parentPoID & ".html#p" & po.PostID)
                    ' finalHtml.Replace("%POST LINK%", ArchivedTHTMLWebPath & parentPoID & ".html#p" & po.PostID)
                Else
                    postHTML = postHTML.Replace("%POST LINK%", pageHandlerName & ".aspx?id=" & parentPoID & "#p" & po.PostID)
                    ' finalHtml.Replace("%POST LINK%", pageHandlerName & ".aspx?id=" & parentPoID & "#p" & po.PostID)
                End If
            Else
                postHTML = postHTML.Replace("%POST LINK%", pageHandlerName & ".aspx?id=" & parentPoID & "#p" & po.PostID)
                'finalHtml.Replace("%POST LINK%", pageHandlerName & ".aspx?id=" & parentPoID & "#p" & po.PostID)
        ...
        '  finalHtml.Replace("%IMAGES%", GetFilesHTML(po))
        ...
        ' If pa.isModerator Or pa.isAdmin Then finalHtml.Replace("%MODPANEL%", GetModeratorHTMLMenu(pa.CredPowers).Replace("%ID%", CStr(po.PostID))) Else finalHtml.Replace("%MODPANEL%", "")
        ...
        If pa.isTrailPost Then
            If commentShortened Then
                ...
                ' finalHtml.Replace("%POST TEXT%", cm)
            Else
                postHTML = postHTML.Replace("%POST TEXT%", cm)
                ' finalHtml.Replace("%POST TEXT%", cm)
            End If
    ...
    Public Sub ProcessPost(ByVal Context As HttpContext)
        ...
        If EnableCaptcha And (mode = "thread" Or mode = "reply") And (Not isAdmin) Then
            Dim re As CaptchaResponse = VerifyCaptcha(Context)
            If Not re.Correct Then
                message = FormatHTMLMessage(errorStr, re.ErrorMessage, "default.aspx", "77", True)
                cont = False
            End If
        End If
        ...
        If cont Then
            If IsIPBanned(Request.UserHostAddress) Then
                Context.Response.Redirect(WebRoot & "banned.aspx")
                Return
            Else
                ...
                Select Case mode
                    Case "thread"
                        ...
                        Else
                            ...
                            If ShowPostSuccessfulMessage Then
                                message = FormatHTMLMessage(SuccessfulPostString, SuccessfulPostString, "default.aspx?id=" & tid, "1", False)
                            Else
                                Context.Response.Redirect(WebRoot & "default.aspx?id=" & tid.ToString())
                                Return
                            End If
                    ...
                    Case "reply"
                        ...
                        If Request.Item("finp") = "yes" And properFiles.Count > 1 Then ' Add each file to a seperate post, and dump the files.
                            ...
                            Else
                                Context.Response.Redirect(WebRoot & "default.aspx?id=" & threadid.ToString() & "#p" & postId.ToString())
                                Return
                            End If
                        ...
                        Else
                            ...
                            If (Request.Item("comment").Length = 0 Or Request.Item("comment").Trim.Length = 0) And properFiles.Count = 0 Then
                                ...
                                message = FormatHTMLMessage("Error", noBlankpost, WebRoot & "default.aspx?id=" & CStr(threadid), "7777", True)
                                Exit Select
                            Else
                    ...
                    Case "report"
                        ...
                        If Request("reportreason") = "" Then
                            Context.Response.Redirect(WebRoot & "report.aspx?id=" & CStr(id) & "&badcap=no")
                        Else
                            If EnableCaptcha Then
                                ...
                                Else
                                    ...
                                    Context.Response.Redirect(WebRoot & "report.aspx?id=" & CStr(id) & "&badcap=yes")
                                End If
                    ...
                    Case "delete"
                        ...
                        If Request("deletepass") = "" Then
                            'REDIRECT TO DELETE POST PAGE
                            Context.Response.Redirect(WebRoot & "deletepost.aspx?id=" & CStr(id))
                            Return
                        Else
                            ...
                            If EnableCaptcha Then
                                ...
                                If re.Correct Then
                                    ...
                                    Else
                                        ...
                                        Context.Response.Redirect(WebRoot & "deletepost.aspx?id=" & CStr(id) & "&badpas=yes")
                                        Return
                                    End If
                                Else
                                    'REDIRECT TO DELETE POST PAGE WITH ERROR CAPTCHA NOTICE
                                    Context.Response.Redirect(WebRoot & "deletepost.aspx?id=" & CStr(id) & "&badcap=yes")
                                    Return
                                End If
    ...
    Friend Function ProcessPostAPI(ByVal context As HttpContext) As ApiResponse
        ...
        If EnableCaptcha And (mode = "thread" Or mode = "reply") And (Not isAdmin) Then
            ...
            If Not re.Correct Then
                ...
                resp.ErrorMessage = re.ErrorMessage
                ContinueProcessing = False
            End If
        End If
    ...
    Friend Function ProcessComment(ByVal po As WPost, ByVal pageHandlerName As String, ByVal para As HTMLParameters) As String
        ...
        Else
            ...
            MatchAndProcessBBCodes("spoiler", sr)
            MatchAndProcessBBCodes("code", sr)
            MatchAndProcessBBCodes("md", sr)
            MatchAndProcessBBCodes("q", sr)
            Return sr
        End If
    End Function
                    ...
                    Else ' It's a video or an audio file.
                        ...
                        Select Case wpi.Extension
                            Case "WEBM"
                                ...
                                If isNext Then scriptItem = scriptItem.Replace("%AN%", "notactive") Else scriptItem = scriptItem.Replace("%AN%", "active")
                                scriptItem = scriptItem.Replace("%filec%", "")
                                items.Append(scriptItem)
    ...
    Public Sub PrunePostFile(ByVal post As WPost, ByVal md5 As String(), Optional ByVal forceDelete As Boolean = False)
        If EnableArchive And Not forceDelete Then
            DeleteFilesFromDB(post.PostID, md5)
            'TODO: Fix this.
        Else
            DeleteFilesFromDB(post.PostID, md5)
        End If

        If AutoDeleteFiles Then
            For Each file As WPostImage In post.files
                If Array.IndexOf(md5, file.MD5) >= 0 Then
                    Delete_File_Fromdisk(IO.Path.Combine(GlobalVariables.StorageFolder, file.ChanbName))
                End If
            Next
        End If
    End Sub
    ...
    Public Function GeneratePageHTML(ByVal isArchive As Boolean, ByVal context As HttpContext) As String
        ...
        If EnableCaptcha And Not isArchive Then
            Dim c As captcha_htmls = GetCaptchaHTML()
            pageHTML = pageHTML.Replace("%CAPTCHA PHOLDER%", c.formHTML)
            pageHTML = pageHTML.Replace("%CAPTCHA PHOLDER QR%", c.qrHTML)
        Else
            pageHTML = pageHTML.Replace("%captcha show%", "hide")
        End If
    ...
    Public Function GenerateCatalogPage(ByVal Request As HttpRequest, ByVal session As HttpSessionState) As String
        ...
        If EnableCaptcha Then
            pageHTML = pageHTML.Replace("%CAPTCHA PHOLDER%", GetCaptchaHTML().formHTML)
        Else
            pageHTML = pageHTML.Replace("%captcha show%", "hide")
        End If
        ...
        pageHTML = pageHTML.Replace("%EXTRA META%", "<link rel=""stylesheet"" href=""" & WebRoot & "css/catalog.css"">")
    ...
    Public Sub GenerateDeleteFilePage(ByVal context As HttpContext)

        If CStr(context.Request("mode")) = "deletefile" Then
            Dim id As Integer = -1
            Try
                id = Math.Abs(CInt(context.Request.Item("id")))
            Catch ex As Exception
                context.Response.Write(FormatHTMLMessage("error", NoPostWasSelected, "", "8888", True))
                Return
            End Try
            ...
            Dim postData As WPost = FetchPostData(id)

            Dim userPass As String = ""
            Try
                userPass = CStr(context.Request("deletepass"))
            Catch ex As Exception
                context.Response.Redirect(WebRoot & "deletefile.aspx?badpas=yes&id=" & id)
                Return
            End Try

            If EnableCaptcha Then

                If Not VerifyCaptcha(context).Correct Then
                    context.Response.Redirect(WebRoot & "deletefile.aspx?badcap=yes&id=" & id & "")
                    Return
                End If

            End If

            If userPass = postData.password Then
                Dim files As New List(Of String)

                For Each x As String In context.Request.Form.Keys
                    If x.StartsWith("file_") Then
                        files.Add(x.Replace("file_", ""))
                    End If
                Next

                If files.Count > 0 Then
                    PrunePostFile(postData, files.ToArray())
                    context.Response.Write(FormatHTMLMessage(sucessStr, "Files deleted", "", "44", False))
                    Return
                Else
                    context.Response.Write(FormatHTMLMessage(errorStr, "No file was selected", "", "44", False))
                    Return
                End If
        ...
        Else
            ...
            If postData.FileCount = 0 Then

                context.Response.Write(FormatHTMLMessage(errorStr, "Post has no files.", "", "44", False))
                Return
            ElseIf postData.FileCount = 1 And postData.comment.Trim().Length = 0 Then

                context.Response.Write(FormatHTMLMessage(errorStr, "Cannot delete this file since the post has no text, delete the post instead.", "", "44", False))
                Return
            ElseIf postData.FileCount = 1 And postData.comment.Trim.Length > 0 Then

                Dim f As WPostImage = postData.files(0)
                If f.ImageThumbailWebPath.Length > 0 Then
                    sb.Append("<li><input class='check' type='checkbox' name='file_%md5%' value='del'><img class='cimage' src='%th%' alt='%fl%' /></input></li>".Replace("%md5%", f.MD5).Replace("%th%", f.ImageThumbailWebPath).Replace("%fl%", f.RealName))
                Else
                    sb.Append("<li><input class='check' type='checkbox' name='file_%md5%' value='del'><span>%fl%</span></input></li>".Replace("%md5%", f.MD5).Replace("%fl%", f.RealName))
                End If

            ElseIf postData.FileCount > 1 Then

                For Each f As WPostImage In postData.files
            ...
            Dim pageHTML As String = DeletePostFileTemplate
            ...
            context.Response.Write(pageHTML.Replace("%ITEMS%", sb.ToString))
            Return
        End If
    End Sub
    ...
    Private Function GenerateCatalogItems(ByVal ids As Integer()) As String
        Dim sb As New StringBuilder
        Dim mobileSb As New StringBuilder
        ...
        '                	</li
        sb.Append("<ul id=""browserlist"" class=""desktop wrap"">")
        mobileSb.Append("<div class=""mobile cati"">")

        Dim mobileTemplate As String = "<div class=""catii""><a href=""%LINK%""> %THUMB% <br/> %TEXT% </a><br/>%STATS%</div><br/>"
        '<img src=""%THUMBSRC%"" /> <br /> <!-- or  --> <span>NO IMAGE</span>
        For Each thread As WPost In data
            sb.Append("<li>")

            Dim mobCopy As String = mobileTemplate
            ...
            If thread.subject = "" Then
                ...
                Else
                    If thread.comment.Length > 50 Then
                        postTitle = thread.comment.Remove(50) & "..."
                    Else
            ...
            'LINK
            mobCopy = mobCopy.Replace("%LINK%", WebRoot & "default.aspx?id=" & CStr(thread.PostID))
            ...
            If i IsNot Nothing Then
                If FileHasThumb(i.Extension) Then
                    ...
                    mobCopy = mobCopy.Replace("%THUMB%", "<img class=""cmi"" src=""$"" />".Replace("$", i.ImageThumbailWebPath))
                Else
                    mobCopy = mobCopy.Replace("%THUMB%", "<span>No image</span>")
            ...
            If ci.TotalReplies > 0 Then
                ...
                sb.Append("</h2>")
                mobCopy = mobCopy.Replace("%STATS%", "R: <b>" & ci.TextReplies & "</b> / I: <b>" & ci.ImageReplies & "</b>")
            Else
                mobCopy = mobCopy.Replace("%STATS%", "<b>No replies</b>")
            ...
            mobCopy = mobCopy.Replace("%TEXT%", postTitle)

            sb.Append("<p class=""website"">View thread</p></a></li>")



            mobileSb.Append(mobCopy)
            mobCopy = Nothing
    ...
    Private Function GenerateGenericHTML() As String
        ...
        pageHTML = pageHTML.Replace("%LANG verificationStr%", Language.verificationStr)
...
#Region "BB codes processing"
    ...
    Private Sub MatchAndProcessBBCodes(ByVal codename As String, ByRef data As String)
                ...
                If data.Contains("[code]") And data.Contains("[/code]") Then
                    Try
                        For Each x In MatchBBCode(data, "code")
                            ...
                            codeLang = codeLang.Trim()
                            If (prC = "") Then
                                data = data.Replace("[code]" & x & "[/code]", "<pre class='prettyprint linenums'>" & codeStr & "</pre>")
                            Else
                                data = data.Replace("[code]" & x & "[/code]", "<pre class='prettyprint linenums lang-" & prC & "'>" & codeStr & "</pre>")
                            End If
                        Next
                        Return
                    Catch ex As Exception
...
#Region "Files database processing"
    ...
    Public Sub DeleteFilesFromDB(ByVal postId As Integer, ByVal md5s As String(), Optional ByVal connection As DbConnection = Nothing)
        Dim myConx As DbConnection

        If connection Is Nothing Then
            myConx = DatabaseEngine.GetDBConnection()
            myConx.Open()
        Else
            myConx = connection
        End If
        For Each x In md5s
            Dim command As DbCommand = DatabaseEngine.GenerateDbCommand(myConx)

            command.CommandText = "DELETE FROM files WHERE (postID = @postID) AND (md5 = @md5)"
            command.Parameters.Add(MakeParameter("@postID", postId, Data.DbType.Int32))
            command.Parameters.Add(MakeParameter("@md5", x, Data.DbType.String))
...
#Region "Captcha Verification"
    Dim captchaProviderName As String = "google"

    Dim google_privateKey As String = "6Ld_0OkSAAAAAAGRZQyLe4-_f8ag5vi4CemCCRak"

    Dim google_publicKey As String = "6Ld_0OkSAAAAAEqHhXtRpQJLQJARRKi-9BVPa8yY"


    Public Function VerifyCaptcha(ByVal context As HttpContext) As CaptchaResponse

        Dim provider As Captcha

        Select Case captchaProviderName

            Case "chanb"
                provider = New ChanbCaptcha()
                Return Nothing
            Case "google"
                provider = New GoogleCaptcha(google_privateKey, google_publicKey)

                Dim captchaData As New CaptchaData()

                captchaData.CaptchaChallenge = If(context.Request.Item("recaptcha_challenge_field") Is Nothing, "", context.Request.Item("recaptcha_challenge_field"))
                captchaData.SolverIP = context.Request.UserHostAddress
                captchaData.SolverResponse = If(context.Request.Item("recaptcha_response_field") Is Nothing, "", context.Request.Item("recaptcha_response_field"))

                Return provider.Verify(captchaData)

            Case Else
                Throw New Exception("INVALID CAPTCHA SERVICE NAME")
                Return Nothing
        End Select
    End Function

    Public Function GetCaptchaHTML() As captcha_htmls
        Select Case captchaProviderName
            Case "chanb"
                Dim p As New ChanbCaptcha()
                Dim i As New captcha_htmls
                i.formHTML = p.FormHTML
                i.qrHTML = p.QrHTML
                Return i
            Case "google"
                Dim p As New GoogleCaptcha(google_privateKey, google_publicKey)
                Dim i As New captcha_htmls
                i.formHTML = p.FormHTML
                i.qrHTML = p.QrHTML
                Return i
    ...
    Friend Class captcha_htmls
        Public formHTML As String = ""
        Public qrHTML As String = ""
    End Class
